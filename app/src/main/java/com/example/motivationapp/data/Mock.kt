...
class Mock {
    ...
    private fun handleLanguage(language: String): List<Phrase> {
        ...
        val phrasesFilter = mListPhrase.filter { it.language == langFilter }

        return phrasesFilter
    }

    fun getRandomPhrase(language: String): String {
        val phrases = handleLanguage(language)
        val randomPhrase = phrases[Random.nextInt(0, phrases.size)]

        return randomPhrase.description
    }

    fun getHappyPhrase(language: String): String {
        val phrases = handleLanguage(language)
        val happyPhrases = phrases.filter { it.categoryId == happy }
        val randomIndex = Random.nextInt(0, happyPhrases.size)

        return happyPhrases[randomIndex].description
    }

    fun getSunnyPhrase(language: String): String {
        val phrases = handleLanguage(language)
        val sunnyPhrases = phrases.filter { it.categoryId == sunny }
        val randomIndex = Random.nextInt(0, sunnyPhrases.size)

        return sunnyPhrases[randomIndex].description
    }
}